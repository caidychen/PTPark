...
@synthesize overlayWindow, hudView, maskType, fadeOutTimer = _fadeOutTimer, stringLabel, imageView, spinnerView, visibleKeyboardHeight;
...
+ (void)showSuccessWithStatus:(NSString *)string {
    [SVProgressHUD showSuccessWithStatus:string duration:0.5];
}
...
- (void)setStatus:(NSString *)string {
    ...
    if(string) {
        ...
        CGSize stringSize = [string boundingRectWithSize:CGSizeMake(200, 300) options:NSStringDrawingTruncatesLastVisibleLine|NSStringDrawingUsesLineFragmentOrigin|NSStringDrawingUsesFontLeading attributes:@{NSFontAttributeName:self.stringLabel.font,                                                                                         NSParagraphStyleAttributeName:paragraphStyle} context:nil].size;
...
- (void)showWithStatus:(NSString*)string maskType:(SVProgressHUDMaskType)hudMaskType networkIndicator:(BOOL)show {
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        [[self overlayWindow] bringSubviewToFront:self];
...
- (void)dismissWithStatus:(NSString*)string error:(BOOL)error {
    [self dismissWithStatus:string error:error afterDelay:0.5];
}
...
+ (void)showWithString:(NSString*)string duration:(NSTimeInterval)duration{
    [SVProgressHUD show];
    [[SVProgressHUD sharedView] dismissWithString:string Indurctor:NO duration:duration];
}

- (void)dismissWithString:(NSString*)string Indurctor:(BOOL)show duration:(NSTimeInterval)duration{
    dispatch_async(dispatch_get_main_queue(), ^{
        if(self.alpha != 1)
            return;
        if(!self.superview)
